<template>
<div>
    <v-calendar title-position="left" class="float-left"/>
                ...
                <table class="table table-sm p-0 my-2" id="frontPanel">
                    ...
                    <tbody>
                        <tr  v-for="room in rooms" :key="room.room_id" >
                            ...
                            <td align="center" v-else-if="room.room_status === 'Occupied'">Occupied</td>
    ...
    export default {
        ...
        methods:{
            ...
            openCheckinModal(data){
                ...
                this.$refs.reserve.reservationData.fill(data.reservation['0']);
                this.$refs.reserve.guest.fill(data.reservation['0'].guest);
                this.$refs.reserve.room.fill(data);
                this.$refs.reserve.rooms = this.rooms;
                $('#reserve').modal('show');
            },
            checkout(data){
                axios.get('api/charge/'+data.room_id)
                    .then(({data})=>(
                        this.$refs.checkout.charges = data
                    ));
                this.$refs.checkout.room_form = data;
                $('#checkout').modal('show');
            },
        ...
        components: {
            CHECKIN,
            DETAILS,RESERVE,EDITGUEST,CHANGEROOM,CHECKOUT
        }
    }
</script>
